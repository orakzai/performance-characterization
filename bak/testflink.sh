...
rm  $outputDir/*
ssh orak@sky2.it.kth.se rm $outputDir/*
...
do
    ...
    if [ "$option" == "w" ];
    then 
        numactl --physcpubind=1 --membind=1 $FLINK_DIR/bin/flink run -q -p 2 -m 130.237.212.71:6123 $FLINK_DIR/examples/WordCount.jar $inputFile  $outputDir/$i
    elif [ "$option" == "p" ];
    then
